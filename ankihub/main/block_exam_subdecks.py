...
from .. import LOGGER
from ..settings import config, BlockExamSubdeckConfig
...
def create_block_exam_subdeck(
    ...
    if due_date:
        config_item = BlockExamSubdeckConfig(
            ankihub_deck_id=str(ankihub_deck_id),
            subdeck_id=str(subdeck_id),
            due_date=due_date
        )
        config.add_block_exam_subdeck(config_item)
    
    LOGGER.info("Created block exam subdeck", 
...
def add_notes_to_block_exam_subdeck(
    ...
    move_notes_to_decks_while_respecting_odid(
        {nid: subdeck["id"] for nid in note_ids}
    )
    ...
    if due_date:
        config_item = BlockExamSubdeckConfig(
            ankihub_deck_id=str(ankihub_deck_id),
            subdeck_id=str(subdeck["id"]),
            due_date=due_date
        )
        config.add_block_exam_subdeck(config_item)
    
    LOGGER.info("Added notes to block exam subdeck",
                subdeck_name=subdeck_name,
