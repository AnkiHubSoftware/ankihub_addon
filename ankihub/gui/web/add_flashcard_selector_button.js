...
function renderFlashCardSelectorButtonOnCongrats() {
    if (!document.getElementById("{{ FLASHCARD_SELECTOR_OPEN_BUTTON_ID }}")) {
        ...
        const congratsWrapper = document.querySelector(".congrats");
        ...
        congratsWrapper.appendChild(button);
...
if (window.location.href.includes("congrats")) {
    waitForElm(".congrats").then(renderFlashCardSelectorButtonOnCongrats);
} else {
    waitForElm("button#study").then(renderFlashCardSelectorButtonOnDecks);
}
