...
class TestCheckBlockExamSubdeckDueDates:
        ...
        with anki_session_with_addon_data.profile_loaded():
            ...
            config_items = [
                BlockExamSubdeckConfig(
                    ankihub_deck_id=str(uuid.uuid4()),
                    subdeck_id=str(uuid.uuid4()),
    ...
    def test_check_block_exam_subdeck_due_dates_mixed_valid_invalid(
        ...
        with anki_session_with_addon_data.profile_loaded():
            ...
            config_items = [
                BlockExamSubdeckConfig(
                    ankihub_deck_id="valid_expired",
                    subdeck_id="subdeck1",
                    due_date=past_date
                ),
                BlockExamSubdeckConfig(
                    ankihub_deck_id="valid_future",
                    subdeck_id="subdeck2",
                    due_date=future_date
                ),
                BlockExamSubdeckConfig(
                    ankihub_deck_id="invalid_date",
                    subdeck_id="subdeck3",
                    due_date="not-a-date"
                ),
            ]
...
class TestTriggerDueDateDialog:
    ...
    def test_trigger_due_date_dialog_with_expired_subdecks(self, mock_showinfo):
        ...
        mock_showinfo.assert_called_once_with(
            "2 block exam subdeck(s) have reached their due date."
        )
    ...
    def test_trigger_due_date_dialog_single_expired(self, mock_showinfo):
        ...
        # Should call showInfo with appropriate message
        mock_showinfo.assert_called_once_with(
            "1 block exam subdeck(s) have reached their due date."
        )
