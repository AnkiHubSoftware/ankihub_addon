...
class TestBlockExamSubdecks:
    """Unit tests for block exam subdeck functionality."""
...
class TestBlockExamSubdeckDialog:
    ...
    def test_successful_subdeck_creation(
        ...
        with anki_session_with_addon_data.profile_loaded():
            ...
            # Verify subdeck was created in Anki
            deck_config = config.deck_config(ah_did)
            anki_deck_name = aqt.mw.col.decks.name_if_exists(deck_config.anki_id)
            full_subdeck_name = f"{anki_deck_name}::New Exam"
            subdeck = aqt.mw.col.decks.by_name(full_subdeck_name)
            assert subdeck is not None

            # Verify notes were moved
            mock_move_notes.assert_called_once()

            # Verify configuration was saved
            saved_due_date = config.get_block_exam_subdeck_due_date(str(ah_did), str(subdeck["id"]))
            assert saved_due_date == future_date.strftime("%Y-%m-%d")
    ...
    def test_subdeck_creation_conflict_handling(
        ...
        with anki_session_with_addon_data.profile_loaded():
            ...
            # Mock no existing subdecks initially
            mocker.patch("aqt.mw.col.decks.children", return_value=[])

            # Create a conflicting subdeck
            deck_config = config.deck_config(ah_did)
            anki_deck_name = aqt.mw.col.decks.name_if_exists(deck_config.anki_id)
            aqt.mw.col.decks.add_normal_deck_with_name(f"{anki_deck_name}::Exam 1")

            dialog = BlockExamSubdeckDialog(ah_did, self.note_ids, parent=None)
            qtbot.addWidget(dialog)

            # Fill in conflicting name
            dialog.name_input.setText("Exam 1")
            future_date = date.today() + timedelta(days=7)
            dialog.date_input.setDate(future_date)
            qtbot.wait(100)

            # Mock the conflict screen
            ...
            mock_conflict_screen.assert_called_once_with("Exam 1")
    ...
    def test_add_to_existing_subdeck_workflow(
        ...
        with anki_session_with_addon_data.profile_loaded():
            ...
            deck_config = config.deck_config(ah_did)
            anki_deck_name = aqt.mw.col.decks.name_if_exists(deck_config.anki_id)
            aqt.mw.col.decks.add_normal_deck_with_name(f"{anki_deck_name}::Existing Exam").id
            ...
            qtbot.mouseClick(dialog.add_notes_button, Qt.MouseButton.LeftButton)
            qtbot.wait(100)
